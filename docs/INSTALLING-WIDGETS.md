...
BarShelf의 URL 설치 v1은 GitHub 저장소, 직접 아카이브, 딥링크를 같은 설치 파이프라인으로 처리한다. 설치된 위젯은 `~/Library/Application Support/barshelf/widgets/<manifest.id>/`에 복사되고, 앱이 실행 중이면 hot reload로 자동 반영된다.
...
```text
https://github.com/example/barshelf-widgets
```

특정 branch 또는 저장소 안의 하위 디렉터리만 설치할 수도 있다.

```text
https://github.com/example/barshelf-widgets/tree/main/widgets/clock
```

BarShelf은 GitHub API를 사용하지 않고, GitHub URL을 다음 다운로드 URL로 변환한다.
...
웹사이트나 README 버튼은 `barshelf://install?url=<percent-encoded-url>` 형식을 사용한다.
...
브라우저에서 `barshelf://install?url=...` 링크를 열면 BarShelf이 설치 확인 화면을 표시한다. 링크 안의 `url` 값은 반드시 percent-encoded URL이어야 한다.
...
기존 위젯과 같은 `manifest.id`를 설치하면 새 설치가 아니라 업데이트로 표시된다. 파일은 `~/Library/Application Support/barshelf/widgets/<manifest.id>/`에 복사된다.

설치가 끝나면 완료 알림에 성공한 위젯 수와 실패 사유가 표시된다. 단일 위젯을 설치했을 때는 완료 알림 대신 팝오버가 자동으로 열리고 방금 설치한 위젯 페이지로 이동해 강조 표시된다.

Widget Gallery에서도 설치된 위젯의 새 버전이 registry에 올라오면 카드 버튼이 **Update**로 바뀐다. 버전이 같으면 기존처럼 "Installed" 상태와 Reinstall 버튼이 표시된다.

## 권한 승인

URL 설치는 파일을 배치할 뿐 권한을 자동 승인하지 않는다. 설치 후 첫 실행 때 BarShelf의 기존 권한 승인 카드가 나타나며, 사용자가 승인해야 위젯이 실행된다.
...
```text
https://github.com/example/barshelf-widgets/tree/release/widgets/clock
```

### 아카이브 안에 위젯이 여러 개 있다

정상이다. BarShelf은 `widget.json`을 포함한 모든 디렉터리를 찾아 멀티 위젯으로 설치한다. 특정 하위 디렉터리만 설치하려면 GitHub `/tree/{branch}/{subdir}` URL을 사용한다.
...
`manifest.id`가 같으면 `~/Library/Application Support/barshelf/widgets/<manifest.id>/` 위치를 업데이트한다. 권한 선언이 같으면 기존 승인 상태가 유지되고, 권한 선언이 바뀌면 다시 승인이 필요하다.

## 번들 위젯 자동 갱신

위젯의 동작은 `widget.json`과 `workflow.json`에 들어 있다 — 즉 **데이터**다.
소수점을 없애거나 경고 임계값을 옮기는 것은 코드 변경이 아니다. 그런데
첫 실행 시딩은 `.seeded-v1` 표식으로 한 번만 일어나기 때문에, 한 번 설치된
위젯은 처음 들어온 버전에 그대로 멈춰 있었다. 앱 릴리스에 고친 위젯을 실어
보내도 아무에게도 닿지 않았다.

BarShelf는 시작할 때, 위젯을 로드하기 직전에 **이미 설치된** 위젯 중 앱
번들이 더 높은 버전을 들고 있는 것만 교체한다. 범위는 일부러 좁다.

- 설치되어 있지 않은 위젯은 설치하지 않는다. 일부러 지운 위젯은 지워진 채로
  남는다.
- 디렉터리 이름이 아니라 `manifest.id`로 맞춘다. 시딩된 스타터는 폴더 이름
  (`today`)으로, 설치된 위젯은 id(`dev.barshelf.today`)로 들어 있기 때문이다.
- 심볼릭 링크는 건너뛴다. 복제한 위젯 인스턴스가 그 형태인데, 실제 디렉터리를
  교체하면 모든 별칭이 새 사본을 가리키게 된다.
- 써 넣은 내용의 지문을 `.bundled-widgets.json`에 남기고, 파일이 그와 달라지면
  손대지 않는다. 직접 고친 위젯은 조용히 덮어쓰지 않는다. 갤러리의 명시적인
  "Update"는 그래도 덮어쓴다.

설정은 `prefs.json`에 위젯 id로 보관하므로 디렉터리를 통째로 교체해도 그대로
남는다. 교체는 설치와 같은 staging/backup 트랜잭션을 쓰므로, 실패하면 이전
위젯이 그대로 살아남는다.

개발 체크아웃(`./widgets/`가 있는 경우)에서는 동작하지 않는다.
