...
Releases의 **별도 에셋 `mbk-<버전>-arm64.tar.gz`**로 배포된다.

```bash
# Releases에서 mbk-<버전>-arm64.tar.gz 와 SHA256SUMS 다운로드 후
shasum -a 256 -c SHA256SUMS --ignore-missing   # 체크섬 검증
tar -xzf mbk-*-arm64.tar.gz
codesign -dv mbk 2>&1 | grep TeamIdentifier    # Developer ID 서명 주체 확인 (728FW73BS8)
sudo mv mbk /usr/local/bin/
mbk --version
```

> **참고 — `spctl`이 mbk를 "Unnotarized"로 표시하는 것은 정상이다.** 단독 CLI
> 바이너리는 스테이플링(ticket 부착) 대상이 아니어서 Gatekeeper 오프라인 평가에
> 공증 티켓이 없다. 서명 주체는 공증된 `BarShelf.app`과 동일한 Developer ID이며,
> 위 `codesign -dv`로 확인할 수 있다.
