...
3. 현재 **v0.3.3은 Developer ID 서명·Apple 공증·티켓 스테이플을 통과한 빌드**이므로 일반 더블클릭으로 실행한다.
4. 메뉴바에 아이콘이 나타나면 클릭 → 온보딩 시작. 로그인 시 자동 실행은 시스템 설정 → 일반 → 로그인 항목에서 추가

> 릴리스 설명과 `SHA256SUMS`를 함께 확인한다. v0.3.3 앱은 최종 ZIP을 다시 풀어 `codesign`, `stapler`, `spctl`, `syspolicy_check` 배포 검사를 통과했다.
...
> v0.3.3 CLI는 Developer ID로 서명되고 Apple 공증 티켓에 각 바이너리의
...
```bash
VERSION=0.3.3 NOTARIZE=0 ALLOW_UNNOTARIZED=1 SIGN_IDENTITY=- bash scripts/release.sh
```
...
> 종료했다가 다시 열어라.
현재 cask는 서명·공증된 v0.3.3 자산과 검증된 SHA-256을 사용한다.
...
| 증상 | 해결 |
|---|---|
| "확인되지 않은 개발자" 경고 | v0.3.3 이상인지와 `SHA256SUMS`를 확인한 뒤 공식 Releases에서 다시 다운로드. 계속되면 이슈에 macOS 버전과 `spctl -a -vv -t exec BarShelf.app` 결과 첨부 |
...
| 단계 | 상태 |
|---|---|
| GitHub Releases | ✅ v0.3.3 서명·공증 산출물 준비 완료 |
...
| Homebrew cask (`brew install --cask barshelf`) | ✅ v0.3.3 체크섬 반영 |
