...
### 번들 위젯을 고쳤다면 그 위젯의 버전도 올린다

앱은 시작할 때 이미 설치된 위젯을 번들 사본으로 교체하는데, **번들 쪽이 더
높은 버전을 선언할 때만** 한다. `workflow.json`만 고치고 `widget.json`의
`version`을 그대로 두면 그 수정은 릴리스에 실려 나가고도 아무에게도 닿지
않는다.

`scripts/check-release-versions.py`가 직전 릴리스 태그와 비교해서 이 실수를
잡는다. 위젯 디렉터리에 변경이 있는데 버전이 그대로면 CI가 실패한다.
