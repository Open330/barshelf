...
이 문서는 BarShelf 위젯 제작자가 GitHub 저장소, `.zip`, `.mbw` 아카이브로 위젯을 배포할 때 따라야 할 구조와 체크리스트를 정리한다.
...
- barshelf CLI 레퍼런스: [`docs/CLI.md`](CLI.md)
- 레지스트리 운영 규약: [`docs/REGISTRY.md`](REGISTRY.md)

## barshelf로 만들기 → 검증 → 패키징

`barshelf` CLI를 쓰면 템플릿 생성부터 `.mbw` 패키징까지 배포 준비를 터미널에서 끝낼 수 있다. 전체 레퍼런스는 [`docs/CLI.md`](CLI.md)에 있다.
...
`barshelf new`는 생성 직후 `barshelf validate`를 자동 실행하므로 템플릿 상태에서 항상 green으로 시작한다. 만들어진 `.mbw` 파일은 GitHub Release 자산 등 임의의 URL에 올려 배포할 수 있고, 사용자는 그 URL을 앱 또는 `barshelf install <url>`로 설치한다.

## 저장소 구조

BarShelf 설치기는 아카이브 안에서 `widget.json`을 포함한 모든 디렉터리를 후보로 찾는다. 단일 위젯 저장소는 루트에 `widget.json`을 둘 수 있다.
...
````markdown
[![BarShelf 설치](https://img.shields.io/badge/BarShelf-Install-0A84FF)](barshelf://install?url=https%3A%2F%2Fgithub.com%2Fexample%2Fbarshelf-widgets)
...
```markdown
[![BarShelf 설치](https://img.shields.io/badge/BarShelf-Install-0A84FF)](barshelf://install?url=https%3A%2F%2Fgithub.com%2Fexample%2Fbarshelf-widgets%2Ftree%2Fmain%2Fwidgets%2Fclock)
```
...
```text
~/Library/Application Support/barshelf/widgets/<manifest.id>/
```

같은 `manifest.id`를 다시 설치하면 업데이트로 처리된다. `version`은 사용자에게 변경을 설명하는 표시값으로 쓰이므로 릴리스마다 올리는 것을 권장한다.

권한 선언이 바뀌면 기존 승인 상태가 유지되지 않는다. BarShelf은 `permissions`의 정규화된 JSON 해시를 저장하므로, `permissions.exec`, `permissions.keychain`, `permissions.notifications`, `permissions.env` 등이 바뀌면 첫 실행 시 다시 승인 카드가 표시된다.
...
- `barshelf validate <위젯 디렉터리>`(또는 pack한 `.mbw`)가 통과한다.
...
1. `barshelf validate <위젯 디렉터리>`가 통과하고, 위 배포 전 체크리스트를 만족하는지 확인한다.
...
```jsonc
{
  ...
  "tags": ["time"],
  "install": { "url": "https://github.com/example/barshelf-widgets/tree/main/widgets/clock" },
  "permissions": { "exec": ["./clock.sh"], "keychain": false, "notifications": false },
  "homepage": "https://github.com/example/barshelf-widgets"
}
```

3. `permissions` 요약은 **표시용**이다. 갤러리 카드에서 사용자에게 위험도를 알리는 신뢰 UX이며, 실제 권한 게이트는 설치 후 첫 실행 승인 카드다. 요약은 manifest 선언과 일치해야 한다.

entry 필드 표, 검증 규칙, 큐레이션 기준(권한 최소화·README 필수 등), PR 리뷰 절차, `BARSHELF_REGISTRY`를 이용한 셀프호스팅은 [`docs/REGISTRY.md`](REGISTRY.md)에, JSON Schema는 [`schema/registry-0.1.json`](../schema/registry-0.1.json)에 있다.
...
사용자 설치 경로까지 확인하려면 위젯 디렉터리를 `~/Library/Application Support/barshelf/widgets/<manifest.id>/`로 복사한다. 같은 `manifest.id`가 `./widgets/`에도 있으면 개발 디렉터리가 우선한다.
