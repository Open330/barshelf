...
| `host.secret.get/set` | `{key, value?}`. Keychain service는 `dev.menubucket`, account는 `<widgetId>/<key>`이다. manifest `permissions.keychain: true`가 필요하다. |
...
- `--import-map=...`: `import { mb, ui } from "menubucket"`을 로컬 SDK 파일로 고정한다.

스크립트는 직접 `Deno.Command`, network, 파일 쓰기를 사용하지 않는다. exec, storage, secret, timer, notification은 모두 `mb.*` API를 통해 호스트에 요청한다.

## SDK API

위젯은 SDK를 import하고 `mb.widget({ load, action, timer })`를 한 번 등록한다.
...
export default mb.widget({
  async load(ctx) {
    await mb.render(ui.text(`Loaded at ${ctx.now}`));
  },
  ...
  async timer(ctx) {
    await mb.log("debug", `timer fired: ${ctx.id}`);
  },
});
```
...
- 모든 context에는 `render`, `exec`, `storage`, `secret`, `timer`, `notify`, `log`, `ui`, `mb`, `reload()`가 포함된다.

### 렌더

```ts
await mb.render(root, {
...
`root`는 `schema/uinode-0.1.json`과 일치하는 UINode이다. `mb.render`는 `host.render` 요청을 보내고 `{revision}`을 반환한다.
...
await mb.storage.delete("count");
const keys = await mb.storage.list("prefix:");
```
...
```ts
const token = await mb.secret.get("token");
await mb.secret.set("token", "value");
```
...
```ts
await mb.timer.once("deadline", Date.now() + 30_000);
await mb.timer.after("retry", 5_000);
await mb.timer.every("minute", 60_000);
await mb.timer.clear("minute");
```
...
```ts
await mb.notify.show({ title: "Done", body: "Widget finished work." });
await mb.log("info", "render complete");
```
...
`ui.*` helper는 `schema/uinode-0.1.json`과 일치하는 JSON 객체를 만든다.

```ts
const root = ui.vstack([
  ui.hstack([
    ui.image("clock", { size: 16, tint: "accent" }),
    ui.text("Script Clock", { role: "title" }),
    ui.spacer(),
    ui.badge("TS", { tone: "neutral" }),
  ], { spacing: 6 }),
  ui.progress(0.5, { label: "Minute", tint: "accent" }),
...
2. `load` handler는 `mb.timer.every("clock-minute", 60_000)`로 1분 timer를 등록한다.
3. 현재 시각과 storage의 클릭 수를 읽어 `mb.render`로 UINode를 보낸다.
