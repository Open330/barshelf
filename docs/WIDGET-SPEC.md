...
| `minHostVersion` | 아니오 | 이 위젯이 도는 가장 오래된 BarShelf 버전(예: `"0.3.11"`). 그 버전에서 생긴 표현식 함수나 소스 필드를 쓸 때 선언한다. 0.3.11 이상 호스트는 이보다 낮으면 설치를 거부하고 실행 대신 "BarShelf X 이상 필요"를 보여 준다. 그 이전 호스트는 이 필드를 무시한다. |
