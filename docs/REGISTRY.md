...
- barshelf CLI: [`docs/CLI.md`](CLI.md)
  ...
  "widgets": [
    {
      ...
      "tags": ["dev", "ai"],
      "category": "Developer",
      "screenshot": "https://raw.githubusercontent.com/OWNER/REPO/main/assets/widget-previews/aas-usage.png",
      "readme": "https://github.com/OWNER/REPO/blob/main/widgets/aas-usage/README.md",
...
| `category` | string | 아니오 | Gallery 필터에 표시할 대표 카테고리. |
| `screenshot` | URI | 아니오 | Gallery 카드에 표시할 HTTP(S) 또는 `file:` 미리보기 이미지. 실제 위젯 렌더 캡처를 권장한다. |
| `readme` | URI | 아니오 | Gallery의 **Details** 버튼으로 여는 Markdown 소개 페이지. GitHub `blob/.../README.md`처럼 렌더링되는 URL을 권장한다. |
| `collection` | string | 아니오 | Gallery shelf 분류. `custom`이면 외부 도구 연동 shelf에 표시된다. |
| `accent` | string | 아니오 | Gallery 카드 아이콘 색상. 위젯 accent 이름 또는 `#RRGGBB`. |
...
### 미리보기와 Markdown 소개 페이지

`screenshot`은 카드 안에서 바로 보이는 작은 이미지다. 실제 BarShelf
렌더러로 만든 PNG를 저장소에 커밋한 뒤 raw URL을 지정하는 방식이 가장
안정적이다. 이 저장소의 공식 미리보기는 다음 명령으로 재생성한다.

```bash
swift run barshelf-app screenshot assets/widget-previews
```

`readme`에는 GitHub의 `blob/.../README.md`처럼 Markdown이 렌더링되는
페이지 URL을 넣는다. 값이 있으면 Gallery 카드에 **Details**가 나타나며,
사용자가 클릭했을 때 기본 브라우저로 연다. 두 필드는 모두 표시용이며
위젯 권한이나 설치 내용에는 영향을 주지 않는다.
...
2. `barshelf validate <위젯 디렉터리>`가 통과하는지 확인한다.
...
5. PR을 올린다. PR 본문에 위젯 저장소 링크, 권한 요약, `screenshot`, `readme`를 포함하는 것을 권장한다.
