...
| Homebrew(`open330/tap`의 `barshelf-cli` formula)로 설치된 CLI | `brew upgrade barshelf-cli`를 안내. Cellar의 **파일은 읽기 전용이지만 디렉터리는 쓰기 가능**하므로 교체 자체는 성공한다 — 그래서 brew는 옛 버전을 갖고 있다고 믿는 상태가 되고, 다음 `brew upgrade`/`reinstall`이 업데이트를 조용히 되돌린다. 기본 PATH에서 `<prefix>/bin`이 `~/.local/bin`보다 앞이라 실제로 실행되는 쪽이 이 복사본이다. |
...
종료 후 다시 열고, **정말로 떴는지 확인한다**.

pid가 있다는 건 증거가 아니다. 실제로 겪은 일이다: 제자리 교체 뒤 새 프로세스가
생겨 pid까지 잡혔지만 커널이 끝내 실행을 허용하지 않았고(32KB, `_dyld_start`에서
정지) 업데이터는 성공을 보고한 뒤 옛 빌드를 종료했다. 그 맥에는 하루 동안
BarShelf가 없었다. 이제 앱은 **메뉴바 아이템을 만든 뒤**
`~/Library/Application Support/barshelf/launch-receipt.json`에 영수증을 남기고,
업데이터는 새 영수증이 나타날 때까지 기다린다. 나타나지 않으면 실패로 보고하고
exit 1이다 — 설치 자체는 끝났다는 점도 함께 알린다.

확인에 실패하면 **교체를 되돌린다**. 새 번들 옆에 이전 번들을 숨겨서 보관해
두었다가, 새 빌드가 신고하면 버리고 아니면 되돌린다. 되돌리기까지 실패하면 그
사실도 구분해서 알린다 — 사용자가 할 일이 달라지기 때문이다.

교체 *전에* 시험 실행하는 방식은 이 문제를 못 잡는다. macOS가 거부한 그 복사본은
**목적지 경로 때문에** 거부된 것이라, 같은 번들을 staging 디렉터리에서 띄우면
멀쩡히 뜬다. 살 곳에서 시험해야 한다.

앱의 **Check for Updates…**도 같은 방식이다. 다만 거기서는 확인될 때까지 **옛
빌드를 종료하지 않는다**. 몇 초 동안 메뉴바 아이콘이 두 개 보이는 대신, 새
빌드가 뜨지 못하면 쓰던 앱이 그대로 남아 상황을 알려준다.
