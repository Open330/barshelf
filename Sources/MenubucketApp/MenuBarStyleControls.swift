...
struct MenuBarStyleControls {
    ...
    /// What this level stores. Weight and colour show it directly: their
    /// "Default" entry means "whatever the layer below says".
    var stored: MenuBarPresentation?
    ...
    var width: some View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            Picker("", selection: Binding(
                ...
                set: { mode in change { $0.width = mode == inherited.effectiveWidth ? nil : mode } }
            )) {
            ...
            case .auto:
                Stepper(value: Binding(
                    ...
                    set: { digits in
                        change { $0.digits = digits == inherited.effectiveDigits ? nil : digits }
                    }
                ), in: 1...6) {
                ...
                .fixedSize()
                hint("Keeps the item the same width as a reading goes from 9 to 10. A longer reading widens it once and it stays that wide.")
            case .fixed:
                ...
                Stepper(value: Binding(
                    ...
                    set: { width in
                        change { $0.valueWidth = width == inherited.effectiveFixedWidth ? nil : width }
                    }
                ), in: 32...120, step: 2) {
                ...
                switch usesOwnItem {
                case true?:
                    hint("The text column is always this wide. Content that does not fit still widens it rather than being cut off.")
                case false?:
                    hint("Fixed needs the item's own place in the menu bar; sharing the BarShelf icon, it keeps a steady width instead.")
                case nil:
                    hint("The text column is always this wide. Items that share the BarShelf icon keep a steady width instead.")
                }
            case .fit:
                hint("Exactly as wide as the current reading, so the item and its neighbours move when the digits change.")
            }
        }
    }
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 10) {
                Picker("", selection: Binding(
                    ...
                    set: { value in change { $0.alignment = value == inherited.effectiveAlignment ? nil : value } }
                )) {
                ...
                Picker("", selection: Binding(
                    ...
                    set: { value in change { $0.size = value == (inherited.size ?? .regular) ? nil : value } }
                )) {
                ...
                Picker("", selection: Binding(
                    get: { stored?.weight?.rawValue ?? "default" },
                    set: { value in change { $0.weight = MenuBarWeight(rawValue: value) } }
                )) {
            ...
            Toggle("Right-align numbers", isOn: Binding(
                ...
                set: { on in
                    ...
                    change { $0.numberAlignment = value == inherited.effectiveNumberAlignment ? nil : value }
                }
            ))
            hint(presentation.effectiveNumberAlignment == .right
    ...
    var color: some View {
        VStack(alignment: .leading, spacing: 4) {
            Picker("", selection: Binding(
                get: { stored?.color ?? "automatic" },
                set: { value in change { $0.color = value == "automatic" ? nil : value } }
            )) {
            ...
            .labelsHidden()
            .frame(width: 150)
            hint("Automatic keeps the widget's own warning colors; monochrome follows the menu bar.")
        }
    }
